...
void init_mqtt_local( ) {
  ...
  add_subtopic(mqtt_GetTopic_P(gv_stopic[2], mqtt_pre_cmnd, gv_clientname, get_stopic_ix(2)), callback_log);
...
void callback_log(String topic, byte* message, unsigned int length) {
  String messageTemp;
  for (int i = 0; i < length; i++) {
    messageTemp += (char)message[i];
  }
  ...
  int lv_log = (int)payload_to_float( message, length);
